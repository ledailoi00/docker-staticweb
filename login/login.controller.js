        ...
        function Verify()
        {
            ...
            AuthenticationService.VerifyNewUserCode(
                ...
                function (response) {
                    ...
                    {
                        toaster.success({title: "Verification Ok", body:"Please use your email address and password to log in to TravelBuddy!", timeout:3000});
                        $scope.mode = 'login';
                        $scope.dataLoading = false;
                        $scope.statusMessage = "Please use your email address and password to log in to TravelBuddy!";
                    }
                    else
                    {
            ...
            AuthenticationService.Login($scope.username, $scope.password, function (response) {
                if (response.succeeded)
                {
                    ...
                    toaster.success(
                        {
                            title: "Login Ok", 
                            body:"Welcome back to TravelBuddy, " + AuthenticationService.CurrentUserInfo().userAttributes.given_name + "!", 
                            timeout:3000
                        });
